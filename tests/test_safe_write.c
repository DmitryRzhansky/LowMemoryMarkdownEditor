...
test_safe_write_file(void)
{
    ...
    LmmeSafeWriteOutcome outcome = lmme_safe_write_file(path,
                                                        "new content",
                                                        strlen("new content"),
                                                        NULL);

    g_assert_cmpint(outcome.result, ==, LMME_SAFE_WRITE_COMMITTED_DURABLE);
    g_assert_true(outcome.fingerprint.exists);
    g_assert_cmpuint(outcome.fingerprint.size, ==, strlen("new content"));
...
test_preserves_permissions(void)
{
    ...
    g_assert_cmpint(lmme_safe_write_file(path, "new", 3, NULL).result,
                    ==,
...
test_does_not_leave_temp_file(void)
{
    ...
    g_assert_cmpint(lmme_safe_write_file(path, "content", 7, NULL).result,
                    ==,
                    LMME_SAFE_WRITE_COMMITTED_DURABLE);
...
test_follows_existing_symlink_without_replacing_it(void)
{
    ...
    g_assert_cmpint(lmme_safe_write_file(link, "new", 3, NULL).result,
                    ==,
                    LMME_SAFE_WRITE_COMMITTED_DURABLE);
...
test_safe_write_fault_targets_invocation(void)
{
    ...
    g_assert_cmpint(lmme_safe_write_file(first, "first", 5, NULL).result,
                    ==,
                    LMME_SAFE_WRITE_COMMITTED_DURABLE);
    g_assert_cmpint(lmme_safe_write_file(second, "second", 6, &error).result,
                    ==,
                    LMME_SAFE_WRITE_NOT_COMMITTED);
    lmme_safe_write_test_reset();
